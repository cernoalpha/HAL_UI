...
class ToplevelWindow(customtkinter.CTkToplevel):
    def __init__(self, *args, **kwargs):
        ...
        self.canvas3 = CTkFrame(self, width=550, height=500)
        self.canvas3.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")


        try:
            image = Image.open("Images/visualization_result.jpg")
            new_width = int(950)
            new_height = int(400)
            ...
            self.image_label = CTkLabel(self.canvas3, image=ctk_image)
            self.image_label.grid(row=0, column=0, sticky="nsew")
            self.image_label.configure(text="Result")
        except Exception as e:
            print("Error displaying image:", e)
        
        self.close_button = CTkButton(self, text="Close", command=self.destroy)
