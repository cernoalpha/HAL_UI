...
def filter_outliers(deviations):
    ...
    print("filtered:" ,filtered_deviations)
    return filtered_deviations

def main():
    image1 = cv2.imread("Img/1.jpg")
    image2 = cv2.imread("Img/2.jpg")
    ...
    # Printing deviations for reference
    deviation_data = {}
    for i, deviation in enumerate(deviations):
        deviation_data[i + 1] = deviation
        print(f"Deviation for dot pair {i + 1}: {deviation:.6f} pixels")

    # Filtered deviations for average calculation
    average_deviation = calculate_average(filtered_deviations)
    print(f"Filtered average deviation: {average_deviation:.6f} pixels")

    visualization = visualize_results(image1.copy(), image2.copy(), matched_pairs_ref, matched_pairs_test)

    cv2.imwrite("visualization_result.jpg", visualization)

    angular_resolution_deg_per_pixel = 0.48529  # Example angular resolution in degrees per pixel
    deviation_arcmin = pixels_to_arcmin(average_deviation, angular_resolution_deg_per_pixel)
    print(f' Deviations in arc min = {deviation_arcmin}')
